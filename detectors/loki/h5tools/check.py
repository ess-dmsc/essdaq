...
def check_keys(data1, data2, keys=None):
    ...
    if keys is None:
        ...
        keys_com = list(set(keys_1).intersection(keys_2))
        if len(list(set(keys_com).intersection(keys))) != len(keys):
            msg.error("Files have different datasets.")
            sys.exit(1)


def check_shapes(data1, data2, keys=None):
