    ...
    def parse_udp_packet(self, packet):
        if UDP in packet:
            ...
            if isinstance(udp_data, Raw):
                ...
                if (data_len == 24):
                    task = lambda data: print(self.__parse_evr_packet(data))
                    self.pool.apply_async(task(udp_bytes))
                ...
                for i in range(0, data_len, 8):
                    ...
                    if(self.__get_readout_type(data_bytes) == 6):
                        task = lambda data: print(self.__parse_tdc_packet(data))
                        self.pool.apply_async(task(data_bytes))
...
sniff(iface="ens2f0.2713", filter="udp and port 9888", prn=parser.parse_udp_packet)
